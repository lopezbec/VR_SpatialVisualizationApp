...
public class CombinatorReferenceViewerRig : MonoBehaviour
{
    public Transform t;
    ...
    private KeyCode[] arrowInputs = { KeyCode.UpArrow, KeyCode.DownArrow, KeyCode.LeftArrow, KeyCode.RightArrow, KeyCode.Z, KeyCode.X };
    ...
    void FixedUpdate()
    {
        ...
        if (!singleRotationMode)
            ...
            for (int i = 0; i < arrowInputs.Length; i++)
            {
                if (Input.GetKey(arrowInputs[i]) && !objectHasRotated)
                {
                    t.Rotate(rotations[i], Space.World);
    ...
    void Update()
    {
        ...
        for (int i = 0; i < arrowInputs.Length; i++)
            if (Input.GetKeyUp(arrowInputs[i]))
